...
# Расчет полинома
    ...
    sus = np.polyval([-2/T**3, 3/T**2, 0, 0], t[-1])
    calculate_position = np.vstack([calculate_position, position[0] + sus*(target_position - position[0])])

    d_sus = np.polyval([-6/T**3, 6/T**2, 0], t[-1])
    dq = J_inv @ (d_sus*(target_position - position[0]))
